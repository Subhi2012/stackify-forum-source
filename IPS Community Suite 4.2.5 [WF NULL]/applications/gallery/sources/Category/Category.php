...
class _Category extends \IPS\Node\Model implements \IPS\Node\Permissions
{
	...
	public function setLastImage( \IPS\gallery\Image $image=NULL )
	{
		if( $image === NULL )
		{
			try
			{
				$image	= \IPS\gallery\Image::constructFromData( \IPS\Db::i()->select( '*', 'gallery_images', array( 'image_category_id=? AND image_approved=1 AND ( image_album_id = 0 OR album_type NOT IN ( 2, 3 ) )', $this->id ), 'image_date DESC', 1, NULL, NULL, \IPS\Db::SELECT_FROM_WRITE_SERVER )->join(
					'gallery_albums',
	...
	public function coverPhoto( $size='thumb' )
	{
		...
		if( !in_array( $size, array( 'masked', 'medium', 'small', 'thumb' ) ) )
		{
	...
	public function clubForm( \IPS\Helpers\Form $form )
	{
		...
		$form->add( new \IPS\Helpers\Form\YesNo( 'category_allow_albums', $this->id ? $this->allow_albums : TRUE ) );
	...
	{
		$this->allow_albums = $values['category_allow_albums'];
