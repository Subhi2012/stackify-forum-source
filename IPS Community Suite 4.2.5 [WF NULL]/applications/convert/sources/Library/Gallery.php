...
class _Gallery extends Core
{
			...
			switch( $k )
			{
				...
				case 'convertClubGalleryCategories':
					$return[ $k ] = array(
						...
						'ips_rows'			=> \IPS\Db::i()->select( 'COUNT(*)', 'gallery_categories', array( "category_club_id IS NOT NULL" ) )->first(),
						'source_rows'		=> $this->software->countRows( $v['table'], $v['where'] ),
						'per_cycle'			=> 1000,
						'dependencies'		=> array( 'convertGalleryImages' ),
				...
				case 'convertClubGalleryImages':
					$return[ $k ] = array(
						...
						'ips_rows'			=> (int) \IPS\Db::i()->select( 'SUM(category_count_imgs)', 'gallery_categories', array( "category_club_id IS NOT NULL" ) )->first(),
						'source_rows'		=> $this->software->countRows( $v['table'], $v['where'] ),
						'per_cycle'			=> 100,
				...
				case 'convertClubGalleryComments':
					$return[ $k ] = array(
						...
						'ips_rows'			=> (int) \IPS\Db::i()->select( 'SUM(category_count_comments)', 'gallery_categories', array( "category_club_id IS NOT NULL" ) )->first(),
						'source_rows'		=> $this->software->countRows( $v['table'], $v['where'] ),
						'per_cycle'			=> 1000,
				...
				case 'convertClubGalleryReviews':
					$return[ $k ] = array(
						...
						'ips_rows'			=> \IPS\Db::i()->select( 'COUNT(*)', 'gallery_reviews', array( \IPS\Db::i()->in( 'review_image_id', \IPS\Db::i()->select( 'image_id', 'gallery_images', array( \IPS\Db::i()->in( 'image_category_id', \IPS\Db::i()->select( 'category_id', 'gallery_categories', array( "category_club_id IS NOT NULL" ) ) ) ) ) ) ) )->first(),
						'source_rows'		=> $this->software->countRows( $v['table'], $v['where'] ),
						'per_cycle'			=> 1000,
						'dependencies'		=> array( 'convertClubGalleryImages' ),
	...
	protected function truncate( $method )
	{
		...
		foreach( $classname::canConvert() as $k => $v )
		{
			...
			{
				case 'convertGalleryCategories':
					$return['convertGalleryCategories'] = array( 'gallery_categories' => NULL, 'core_clubs_node_map' => array( "node_class=?", "IPS\\gallery\\Category" ) );
					break;
