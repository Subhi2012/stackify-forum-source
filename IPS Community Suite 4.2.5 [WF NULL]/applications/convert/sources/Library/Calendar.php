...
class _Calendar extends Core
{
			...
			switch( $k )
			{
				...
				case 'convertCalendarVenues':
					$return[ $k ] = array(
						...
						'ips_rows'			=> \IPS\Db::i()->select( 'COUNT(*)', 'calendar_venues' )->first(),
						'source_rows'		=> $this->software->countRows( $v['table'], $v['where'] ),
						'per_cycle'			=> 100,
				...
				case 'convertCalendarReminders':
					$return[ $k ] = array(
						...
						'ips_rows'			=> \IPS\Db::i()->select( 'COUNT(*)', 'calendar_event_reminders' )->first(),
						'source_rows'		=> $this->software->countRows( $v['table'], $v['where'] ),
						'per_cycle'			=> 100,
						'dependencies'		=> array( 'convertCalendarEvents' ),
	...
	protected function truncate( $method )
	{
		...
		foreach( $classname::canConvert() as $k => $v )
		{
			...
			{
				case 'convertCalendarCalendars':
					$return['convertCalendarCalendars'] = array( 'calendar_calendars' => NULL, 'core_clubs_node_map' => array( "node_class=?", "IPS\\calendar\\Calendar" ) );
					break;
