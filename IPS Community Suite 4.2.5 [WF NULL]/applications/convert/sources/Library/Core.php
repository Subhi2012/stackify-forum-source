...
class _Core extends \IPS\convert\Library
{
	...
	protected function truncate( $method )
	{
		...
		foreach( $classname::canConvert() as $k => $v )
		{
			switch( $k )
			{
				...
				case 'convertGroups':
					$return['convertGroups'] = array( 'core_groups' => array( \IPS\Db::i()->in( 'g_id', array( \IPS\Settings::i()->member_group, \IPS\Settings::i()->guest_group, \IPS\Settings::i()->admin_group ), TRUE ) ), 'core_group_promotions' => NULL );
					break;
	...
	public function convertAttachment( $info=array(), $map=array(), $filepath=NULL, $filedata=NULL, $thumbnailpath=NULL )
	{
		...
		try
		{
			...
			\IPS\File\FileSystem::$converterCopy = TRUE;

			/* Create the file */
			$file = \IPS\File::create( 'core_Attachments', $info['attach_file'], $filedata, $container, TRUE, $filepath, static::$obscureFilenames );

			/* Revert file system to default functionality */
			\IPS\File\FileSystem::$converterCopy = FALSE;
			unset( $filedata );
		...
		if( $thumbnailpath AND isset( $info['attach_thumb_location'] ) AND $info['attach_thumb_location'] )
		{
			...
			try
			{
				...
				/* Create the file */
				$thumbnail = \IPS\File::create( 'core_Attachments', $thumbFilename, NULL, $thumbnailContainer, TRUE, $thumbnailpath, static::$obscureFilenames );
	...
	public function convertFollow( $info=array() )
	{
		...
		$info['follow_index_id']	= ( isset( $info['follow_index_id'] ) ) ? $info['follow_index_id'] : NULL;
	...
	public function convertMember( $info=array(), $profileFields=array(), $profilePhotoName=NULL, $profilePhotoPath=NULL, $profileFileData=NULL, $coverPhotoName=NULL, $coverPhotoPath=NULL, $coverFileData=NULL )
	{
		...
		if( isset( $info['fb_uid'] ) )
		{
			$info['fb_uid'] = (int) $info['fb_uid'];
		}
	...
	public function convertMemberHistory( $info=array() )
	{
		...
		switch( $info['log_type'] )
		{
			...
			case 'group_promotion_o':
				$groups = array();
				foreach( explode( ',', $info['log_data']['new'] ) as $groupId )
				{
					try
					{
