...
class _Blog extends \IPS\Node\Model implements \IPS\Node\Ratings, \IPS\Content\Embeddable
{
	...
	*/
	...
	{
		$title = htmlspecialchars( $containerData['blog_name'], ENT_QUOTES | \IPS\HTMLENTITIES, 'UTF-8', FALSE );
		return ( $containerData['blog_member_id'] ) ? $title : parent::titleFromIndexData( $indexData, $itemData, $containerData );
	}
	...
	public static function followWhere( &$joins )
	{
		...
		if ( \IPS\Settings::i()->clubs )
		{
			...
			if ( \IPS\Member::loggedIn()->member_id )
            {
				$where[] = array( '( blog_blogs.blog_club_id IS NULL OR ' . \IPS\Db::i()->in( 'blog_blogs.blog_club_id', \IPS\Member::loggedIn()->clubs() ) . ' OR core_clubs.type IN(?,?) )', \IPS\Member\Club::TYPE_PUBLIC, \IPS\Member\Club::TYPE_OPEN );
            }
            else
            {
				$where[] = array( '( blog_blogs.blog_club_id IS NULL OR core_clubs.type IN(?,?) )', \IPS\Member\Club::TYPE_PUBLIC, \IPS\Member\Club::TYPE_OPEN );
            }
		}
	...
	public function setLastComment( \IPS\Content\Comment $comment=NULL )
	{
		...
		if ( $latestEntry = $this->latestEntry( \IPS\Db::SELECT_FROM_WRITE_SERVER ) )
		{
	 ...
	 * Get latest entry
	 *
	 * @param	int		$sqlFlags		SQL bitwise flags to pass, see \IPS\Db::select()
	 * @return	\IPS\blog\Entry|NULL
	 */
	public function latestEntry( $sqlFlags=0 )
	{
		...
		try
		{
			...
			$this->latestEntry = \IPS\blog\Entry::constructFromData( \IPS\Db::i()->select( '*', 'blog_entries', array( 'entry_blog_id=? AND entry_is_future_entry=0 AND entry_hidden=1 AND entry_status!=?', $this->_id, 'draft' ), 'entry_date DESC', 1, NULL, NULL, $sqlFlags )->first() );
			return $this->latestEntry;
		}
		catch ( \UnderflowException $e )
		{
	...
	{
		/* Parent method performs escaping to prevent XSS */
		$title = parent::get__title();

		return $this->member_id ? $this->name : $title;
	}
	...
	public function clubForm( \IPS\Helpers\Form $form )
	{
		...
		$form->add( new \IPS\Helpers\Form\Editor( 'club_node_description', $this->_id ? $this->description : NULL, FALSE, array( 'app' => 'blog', 'key' => 'Blogs', 'autoSaveKey' => ( $this->id ? "blogs-blog-{$this->id}" : "blogs-new-blog" ), 'attachIds' => $this->id ? array( $this->id, NULL, 'description' ) : NULL, 'minimize' => 'blog_desc_placeholder' ) ) );
		if( \IPS\Settings::i()->blog_allow_rss )
		{
			$form->add( new \IPS\Helpers\Form\YesNo( 'blog_enable_rss', $this->id ? $this->settings['allowrss'] : TRUE ) );
		}
