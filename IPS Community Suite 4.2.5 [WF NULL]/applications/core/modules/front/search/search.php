		...
		foreach ( $contentTypes as $k => $class )
		{
			if ( isset( $class::$containerNodeClass ) )
			{
				...
				if( isset( \IPS\Request::i()->type ) AND !empty( \IPS\Request::i()->type ) )
				{
					...
					$typeClass	= 'IPS\\' . $typeInfo[0] . '\\' . ucfirst( $typeInfo[1] );
				...
				$field = new \IPS\Helpers\Form\Node( $k . '_node', $nodes, FALSE, array( 'class' => $nodeClass, 'multiple' => TRUE, 'permissionCheck' => $nodeClass::searchableNodesPermission(), 'forceOwner' => FALSE, 'clubs' => \IPS\IPS::classUsesTrait( $nodeClass, 'IPS\Content\ClubContainer' ) ), NULL, NULL, NULL, $k . '_node' );
