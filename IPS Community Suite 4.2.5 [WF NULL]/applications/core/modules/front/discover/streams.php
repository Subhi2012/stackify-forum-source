...
class _streams extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		{
			$streamFields = array( 'include_comments', 'classes', 'clubs', 'ownership', 'custom_members', 'read', 'follow', 'followed_types', 'date_type', 'date_start', 'date_end', 'date_relative_days', 'sort', 'tags', 'unread_links' );
		...
		if ( !isset( \IPS\Request::i()->id ) )
		{
			...
			$extra = array();
			foreach ( array( 'register', 'follow_member', 'follow_content', 'photo', 'like', 'react', 'clubs' ) as $k )
			{
				$key = "all_activity_{$k}";
	...
	protected function _buildForm( \IPS\core\Stream &$stream )
	{
		...
		if ( $values = $form->values() )
		{
			...
			if ( isset( \IPS\Request::i()->updateOnly ) )
			{
				...
				foreach ( array( 'include_comments', 'classes', 'containers', 'clubs', 'ownership', 'custom_members', 'read', 'follow', 'followed_types', 'date_type', 'date_start', 'date_end', 'date_relative_days', 'sort', 'tags', 'unread_links' ) as $k )
				{
					$requestKey = 'stream_' . $k;
