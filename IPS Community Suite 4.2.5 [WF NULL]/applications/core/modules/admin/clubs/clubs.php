...
{	
	...
	{			
		...
		{			
			...
			/* Settings Button */
			if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'core', 'clubs', 'fields_manage' ) )
			{
				\IPS\Output::i()->sidebar['actions']['fields'] = array(
					'icon'	=> 'cog',
					'title'	=> 'clubs_custom_fields',
					'link'	=> \IPS\Http\Url::internal( "app=core&module=clubs&controller=clubs&controller=fields" ),
				);
			}
			if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'core', 'clubs', 'clubs_settings_manage' ) )
			{
				\IPS\Output::i()->sidebar['actions']['settings'] = array(
					'icon'	=> 'cog',
					'title'	=> 'settings',
					'link'	=> \IPS\Http\Url::internal( "app=core&module=clubs&controller=clubs&controller=settings" ),
				);
			}
				...
				foreach ( $values as $k => $v )
				{
					...
					try
					{
						...
						{					
							if ( $v )
							{
								\IPS\Task::queue( 'core', 'DeleteOrMoveContent', array( 'class' => $nodeClass, 'id' => $_node->_id, 'moveToClass' => $nodeClass, 'moveTo' => $v->_id, 'deleteWhenDone' => TRUE, 'additional' => array() ) );
							}
							else
							{
								\IPS\Task::queue( 'core', 'DeleteOrMoveContent', array( 'class' => $nodeClass, 'id' => $_node->_id, 'deleteWhenDone' => TRUE, 'additional' => array() ) );
							}
						}
					}
					catch ( \Exception $e ) {}
