...
class _advanced extends \IPS\Dispatcher\Controller
{
	...
	protected function downloadDatastoreConstants()
	{
		...
		{
			$val = constant( 'IPS\\' . $k );
			if ( $val !== $v and !in_array( $k, array( 'STORE_METHOD', 'STORE_CONFIG', 'CACHE_METHOD', 'CACHE_CONFIG', 'CACHE_PAGE_TIMEOUT', 'SUITE_UNIQUE_KEY', 'READ_WRITE_SEPARATION' ) ) )
			{
	...
	protected function _manageSettings()
	{
		...
		{
			\IPS\Settings::i()->changeValues( array( 'task_cron_key' => md5( uniqid() ) ) );
		}
