...
class _leaderboard extends \IPS\Task
{
	...
	public function execute()
	{
		...
		for( $i = 0; $i < $difference; $i++ )
		{
			...
			foreach( \IPS\Db::i()->select( 'core_reputation_index.member_received as member, SUM(rep_rating) as rep', 'core_reputation_index', array( 'member_received > 0 AND rep_date BETWEEN ? and ?', $startDate->getTimeStamp(), $startDate->setTime( 23, 59, 59 )->getTimeStamp() ), 'rep DESC', 4, 'member' )->setKeyField('member')->setValueField('rep') as $member => $rep )
			{
			...
			/* Remove any potental blanks */
			\IPS\Db::i()->delete( 'core_reputation_leaderboard_history', array( 'leader_position > ? and leader_date BETWEEN ? and ?', $position, $date, $date - 4 ) );
