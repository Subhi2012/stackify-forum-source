...
class _Lastpost
{
	...
	public function getQueryWhereClause( $data )
	{
		...
		{
			$start	= ( $data['range']['start']['date'] ) ? new \IPS\DateTime( $data['range']['start']['date'] ) : NULL;
			$end	= ( $data['range']['end']['date'] ) ? new \IPS\DateTime( $data['range']['end']['date'] ) : NULL;

			if( $start and $end )
			{
				return "core_members.member_last_post BETWEEN {$start} AND {$end}";
			}
		}
		elseif( !empty($data['days']) )
		{
			...
			return "core_members.member_last_post < {$date->getTimestamp()}";
		}
