...
class _Member extends \IPS\Patterns\ActiveRecord
{
	...
	public function save()
	{
		...
		/* Check Changes for Member History */
		$this->logChanges( $changes, $new );
	...
	/**
	 * Log changes from the save method
	 *
	 * @param	array		$changes
	 * @return	void
	 */
	protected function logChanges( $changes, $new=FALSE )
	{
		$logTypes = array( 'member_group_id', 'mgroup_others', 'name', 'linkedin_id', 'fb_uid', 'google_id', 'live_id', 'twitter_id', 'ipsconnect_id' );

		foreach( $changes as $field => $value )
		{
			if( !in_array( $field, $logTypes ) )
			{
				continue;
			}

			/* Don't log anything if it's a new account, except if it's a social service connection */
			if( $new and !in_array( $field, array( 'linkedin_id', 'fb_uid', 'google_id', 'live_id', 'twitter_id', 'ipsconnect_id' ) ) )
			{
				continue;
			}
			...
			switch( $field )
			{
				case 'member_group_id':
					if( $this->_groupPromotion === NULL )
					{
						$this->logHistory( 'core', $field, array( 'new' => $value ) );
					}
					else
					{
						$this->logHistory( 'core', 'group_promotion', array( 'reason' => $this->_groupPromotion, 'new' => $value ) );
					}
					break;
				...
				case 'mgroup_others':
					...
					{
						$this->logHistory( 'core', 'group_promotion_o', array( 'reason' => $this->_groupPromotion, 'new' => $value ) );
					}
					break;
				case 'name':
					$this->logHistory( 'core', 'display_name', array( 'old' => $this->previousName, 'new' => $value ) );
					break;
				case 'linkedin_id':
				case 'fb_uid':
				case 'google_id':
				...
				case 'twitter_id':
				case 'ipsconnect_id':
					/* expand the name so it can be used later for language lookups */
					if( $field == 'fb_uid' )
					{
						$field = 'facebook';
					}
					
					$data = array( 'service' => str_replace( array( '_uid', '_id' ), '', $field ), 'linked' => ( (bool) $value ) );
					...
					$this->logHistory( 'core', 'social_account', $data );
					break;
				default:
					$this->logHistory( 'core', $field, array( 'new' => $value ) );
					break;
	...
	public static function photoUrl( $memberData, $thumb=TRUE, $email=FALSE, $useDefaultPhoto=TRUE )
	{
		...
		if ( isset( $memberData['member_id'] ) and $memberData['member_id'] )
		{
			...
			if ( $memberData['pp_main_photo'] and ( mb_substr( $memberData['pp_photo_type'], 0, 5 ) === 'sync-' or $memberData['pp_photo_type'] === 'custom' or ( \IPS\Settings::i()->letter_photos == 'letters' AND $memberData['pp_photo_type'] == 'letter' AND ( $gravatarDisabled OR !\IPS\Settings::i()->allow_gravatars ) ) ) )
			{
				try
				{
			...
			if ( \IPS\Dispatcher::hasInstance() AND \IPS\Dispatcher::i()->controllerLocation === 'admin' AND ( $photoUrl instanceof \IPS\Http\Url ) )
			{
	...
	{
		/* This theme has letter-photos enabled and this user has no photo, so generate a new letter photo if possible. */
		if( \IPS\Settings::i()->letter_photos == 'letters' AND \IPS\Image::canWriteText() AND isset( $memberData['member_id'] ) AND $memberData['member_id'] )
		{
			...
			$letterPhoto = new \IPS\Member\LetterPhoto( $member->name );
	...
	/**
	 * @brief	Group Promotion Type
	 */
	protected $_groupPromotion = NULL;
	...
	public function checkGroupPromotion()
	{
		...
		{
			$this->_groupPromotion = $ruleToUse->id;
	...
	{
		$newKey = uniqid();
		\IPS\Settings::i()->changeValues( array( 'member_menu_create_key' => $newKey ) );
	}
	...
	public function invalidateSessionsAndLogins( $frontEndSessions=TRUE, $acpSessions=TRUE, $loginKeys=TRUE )
	{
		...
		{
			$where = array( array( 'member_id=?', $this->member_id ) );
			if ( is_string( $frontEndSessions ) )
			{
				$where[] = array( 'id<>?', $frontEndSessions );
			}
	 ...
	 *
	 * @param	$record		validating record
	 * @return	void
	 */
	...
	public function logHistory( $app, $type, $extra=NULL, $by=NULL )
	{
		if ( $this->member_id )
		{
			if ( $by === NULL )
			{
				$by = ( !\IPS\Dispatcher::hasInstance() ) ? NULL : \IPS\Member::loggedIn();
			}
	...
	public static function topMembers( $type, $limit )
	{
		...
		else
		{
			...
			if ( ! $stored or ( ( time() - $stored['time'] ) > 300 ) )
			{
				...
				$memberIds = array();
				foreach( \IPS\Db::i()->select( 'member_id', 'core_members', \IPS\Db::i()->in( 'member_id', array_keys( $members ) ) ) AS $member_id )
				{
