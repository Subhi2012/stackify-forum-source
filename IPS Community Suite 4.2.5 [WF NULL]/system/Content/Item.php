...
abstract class _Item extends \IPS\Content
{
	...
	public function contentImages( $limit = NULL )
	{
		...
		{
			$internal = \IPS\Db::i()->select( 'attachment_id', 'core_attachments_map', array( 'location_key=? and id1=?', static::$application . '_' . ucfirst( static::$module ), $this->$idColumn ) );
		}
	...
	public function metaDescription()
	{
		...
		if ( $return )
		{
			$return =  preg_replace( '#\n{1,}#', "\n", trim( str_replace( '&nbsp;', ' ', strip_tags( preg_replace('#(<(script|style)\b[^>]*>).*?(</\2>)#is', "$1$3", $return ) ) ) ) );
	...
	{		
		...
		$comments[ $_hash ]	= $this->_comments( $class, $limit ?: $this->getCommentsPerPage(), $offset, ( isset( $class::$databaseColumnMap[ $order ] ) ? ( $class::$databasePrefix . $class::$databaseColumnMap[ $order ] ) : $order ) . ' ' . $orderDirection, $member, $includeHiddenComments, $cutoff, NULL, $extraWhereClause, $includeDeleted );
		return $comments[ $_hash ];
	}
	 ...
	 * @return	array|NULL|\IPS\Content\Comment	If $limit is 1, will return \IPS\Content\Comment or NULL for no results. For any other number, will return an array.
	 */
	public function reviews( $limit=NULL, $offset=NULL, $order=NULL, $orderDirection='desc', $member=NULL, $includeHiddenReviews=NULL, $cutoff=NULL, $extraWhereClause=NULL, $includeDeleted=FALSE )
	{
		...
		$this->cachedReviews[ $cacheKey ]	= $this->_comments( $class, $limit ?: static::$reviewsPerPage, $offset, $order, $member, $includeHiddenReviews, $cutoff, NULL, $extraWhereClause, $includeDeleted );
		return $this->cachedReviews[ $cacheKey ];
	}
	...
	protected function _comments( $class, $limit, $offset=NULL, $order='date DESC', $member=NULL, $includeHidden=NULL, $cutoff=NULL, $canViewWarn=NULL, $extraWhereClause=NULL, $includeDeleted=FALSE )
	{
		...
		if ( \IPS\IPS::classUsesTrait( $class, 'IPS\Content\Reactable' ) AND count( $results ) )
		{
			/* Work out the query */
			$reputationWhere = array( array( 'core_reputation_index.rep_class=? AND core_reputation_index.type=? AND core_reactions.reaction_enabled=?', $class::reactionClass(), $class::reactionType(), 1 ) );
			$reputationWhere[] = array( \IPS\Db::i()->in( 'core_reputation_index.type_id', array_keys( $results ) ) );
			
			$names = array();
			$reactions = array();
			$select = \IPS\Db::i()->select( 'core_reputation_index.type_id, core_reputation_index.member_id, core_reputation_index.rep_rating, core_reputation_index.reaction, core_members.name, core_members.members_seo_name', 'core_reputation_index', $reputationWhere, 'RAND()' )->join( 'core_members', 'core_members.member_id=core_reputation_index.member_id', 'INNER' )->join( 'core_reactions', 'core_reputation_index.reaction=core_reactions.reaction_id' );
			...
			foreach ( $select as $reputation )
			{
				...
				@uksort( $reactions[ $reputation['type_id'] ], function( $a, $b ) {
	...
	public function canHide( $member=NULL )
	{
		...
		$member = $member ?: \IPS\Member::loggedIn();
		return ( $member->member_id and ( static::modPermission( 'hide', $member, $container ) or ( $member->member_id == $this->author()->member_id and ( $member->group['g_hide_own_posts'] == '1' or in_array( get_class( $this ), explode( ',', $member->group['g_hide_own_posts'] ) ) ) ) ) );
	}
	...
	public function canReport( $member=NULL )
	{
		...
		$member = $member ?: \IPS\Member::loggedIn();
		return ( $member->group['g_can_report'] == '1' or in_array( get_class( $this ), explode( ',', $member->group['g_can_report'] ) ) );
	}
	 ...
	 * @param	\IPS|Member|NULL	User adding the message
	 * @return	int
	 * @note This is a wrapper for the extension so content items can extend and apply their own logic
	 */
