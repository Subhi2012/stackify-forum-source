	...
	public static function publish( $content, $url=null )
	{
		if ( static::canAutoshare() )
		{
			try
			{
				...
				if ( $url !== NULL )
				{
					$response = \IPS\Http\Url::external( "https://graph.facebook.com/" . \IPS\Member::loggedIn()->fb_uid . "/feed" )->setQueryString( 'appsecret_proof', \IPS\Login\Facebook::appSecretProof( $this->member->fb_token ) )->request()->post( array(
						'message'		=> $content,
						'link'			=> (string) $url,
			...
			{
				\IPS\Log::log( \IPS\Member::loggedIn()->id . ': '. $e->getMessage(), 'facebook' );
