...
class _Facebook extends PromoteAbstract
{
	...
	public function canPostToPage()
	{
		try
		{
			$response = \IPS\Http\Url::external( "https://graph.facebook.com/{$this->member->fb_uid}/permissions" )->setQueryString( array( 'access_token' => $this->member->fb_token, 'appsecret_proof' => \IPS\Login\Facebook::appSecretProof( $this->settings['token'] ) ) )->request()->get()->decodeJson();
	...
	public function getPages()
	{
		...
		try
		{
			$response = \IPS\Http\Url::external( "https://graph.facebook.com/{$this->member->fb_uid}/accounts" )->setQueryString( array( 'access_token' => $this->member->fb_token, 'appsecret_proof' => \IPS\Login\Facebook::appSecretProof( $this->settings['token'] ) ) )->request()->get()->decodeJson();
