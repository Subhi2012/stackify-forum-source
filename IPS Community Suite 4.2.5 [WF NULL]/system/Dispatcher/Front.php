...
class _Front extends \IPS\Dispatcher\Standard
{
	...
	public function init()
	{
		...
		if( \IPS\Member::loggedIn()->member_id and $this->controller !== 'language' and $this->controller !== 'theme' and $this->controller !== 'ajax' )
		{
			...
			else if ( \IPS\Settings::i()->quick_register AND !\IPS\Member::loggedIn()->members_bitoptions['profile_completed'] AND !in_array( $this->controller, array( 'register', 'login', 'redirect', 'contact', 'privacy', 'ajax', 'settings' ) ) AND $completion = \IPS\Member::loggedIn()->profileCompletion() AND count( $completion['required'] ) )
			{
	...
	protected function checkCached()
	{
		...
		if ( \IPS\STORE_METHOD === 'Database' AND ( !\IPS\Data\Cache::i() instanceof \IPS\Data\Cache\None ) )
		{
