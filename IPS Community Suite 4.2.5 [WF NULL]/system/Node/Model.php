...
abstract class _Model extends \IPS\Patterns\ActiveRecord
{
	 ...
	 * @return	array
	 */
	protected static function nodesWithPermission( $permissionCheck, $member, $where=array() )
	{
		/* Permission check? */
		if ( $permissionCheck )
		{
	...
	public static function search( $column, $query, $order, $where=array() )
	{
		if ( $column === '_title' AND static::$titleLangPrefix !== NULL )
		{
			...
			{
				/* Is this a description? If so, exclude - easier to do in PHP than SQL */
				if( isset( static::$descriptionLangSuffix ) AND static::$descriptionLangSuffix )
				{
					if( mb_strpos( $key, static::$descriptionLangSuffix ) !== FALSE )
					{
						continue;
					}
				}
				...
				try
				{
					$nodeObject		= static::load( $key );
					$parent			= NULL;

					try
					{
						while( $parent = $nodeObject->parent() )
						{
							$nodeObject = $parent;
						}
					}
					...
					$return[ $parent ? $parent->_id : $key ] = $parent ?: $nodeObject;
				}
				catch ( \OutOfRangeException $e ) { }
			}
	...
	public static function basicDataColumns()
	{
		...
		{
			$return[] = static::$featureColumnName;
		}
	 ...
	 *
	 * @param	$properties				Array of property=>value i.e. array( 'redirect_url' => FALSE, 'password' => FALSE );
	 * @param	$returnRoots			Enable to check and return the root node
	 * @param	$subNodes				Enable to check subnodes
	...
	public static function theOnlyNode( $properties=array(), $returnRoots=TRUE, $subNodes=TRUE )
	{
		...
		/* Make sure the nodes are not club nodes */
		if ( \IPS\IPS::classUsesTrait( get_called_class(), 'IPS\Content\ClubContainer' ) )
		{
			$where[] = array( static::$databasePrefix . static::clubIdColumn() . ' IS NULL' );
		}

		if ( count( static::roots( 'view', NULL, $where ) ) === 1 )
		{
			foreach ( static::roots( 'view', NULL, $where ) as $root )
			{
				if ( $root->childrenCount( 'view', NULL, $subNodes ) === 1 )
				{
					foreach ( $root->children( 'view', NULL, $subNodes ) as $node )
					{
						...
						/* If there are children here just return NULL */
						if( $node->childrenCount( 'view', NULL, $subNodes ) )
						{
							return NULL;
						}
	...
	protected function get__title()
	{
		...
		{
			$options = array();
			if ( \IPS\IPS::classUsesTrait( $this, 'IPS\Content\ClubContainer' ) )
			{
				$clubColumnId = $this->clubIdColumn();
				if ( $this->$clubColumnId )
				{
					$options = array( 'escape' => TRUE );
				}
			}
			return \IPS\Member::loggedIn()->language()->addToStack( static::$titleLangPrefix . $this->_id, NULL, $options );
		}
		return '';
	}
	...
	public static function canOnAny( $permission, $member=NULL )
	{
		...
		if ( static::$databaseColumnEnabledDisabled )
		{
			static::$_canOnAny[ $_key ][ $permission ]	= (bool) \IPS\Db::i()->select( 'COUNT(*)', static::$databaseTable, array( static::$databasePrefix . static::$databaseColumnEnabledDisabled . '=1 AND (' . \IPS\Db::i()->findInSet( 'core_permission_index.perm_' . static::$permissionMap[ $permission ], $member->groups ) . ' OR core_permission_index.perm_' . static::$permissionMap[ $permission ] . "='*' )" ) )
				->join( 'core_permission_index', array( "core_permission_index.app=? AND core_permission_index.perm_type=? AND core_permission_index.perm_type_id=" . static::$databaseTable . "." . static::$databasePrefix . static::$databaseColumnId, static::$permApp, static::$permType ) )
				->first();
		}
		else
		{
			static::$_canOnAny[ $_key ][ $permission ]	= (bool) \IPS\Db::i()->select( 'COUNT(*)', 'core_permission_index', array( 'core_permission_index.app=? AND core_permission_index.perm_type=? AND (' . \IPS\Db::i()->findInSet( 'core_permission_index.perm_' . static::$permissionMap[ $permission ], $member->groups ) . ' OR core_permission_index.perm_' . static::$permissionMap[ $permission ] . "='*' )", static::$permApp, static::$permType ) )->first();
		}
	...
	public function can( $permission, $member=NULL )
	{
		...
		if( $member instanceof \IPS\Member\Group )
		{
			return ( $permissions[ 'perm_' . static::$permissionMap[ $permission ] ] === '*' or ( $permissions[ 'perm_' . static::$permissionMap[ $permission ] ] and in_array( $member->g_id, array_filter( explode( ',', $permissions[ 'perm_' . static::$permissionMap[ $permission ] ] ) ) ) ) );
		}
		else
		{			
			return ( $permissions[ 'perm_' . static::$permissionMap[ $permission ] ] === '*' or ( $permissions[ 'perm_' . static::$permissionMap[ $permission ] ] and array_intersect( array_filter( explode( ',', $permissions[ 'perm_' . static::$permissionMap[ $permission ] ] ) ), $member->permissionArray() ) ) );
		}
	}
	 ...
	 * @param	string|NULL					$contentClass	The content class to check
	 * @return	bool
	 */
	public function modPermission( $type, \IPS\Member $member, $contentClass = NULL )
	{
		if ( isset( static::$modPerm ) )
		{
			$itemClass = $contentClass ?: static::$contentItemClass;
