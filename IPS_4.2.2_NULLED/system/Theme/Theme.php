...
EOF;
	...
	public function saveCss( $data )
	{
		...
		/* Unique key */
		$css['css_unique_key'] = md5( $data['set_id'] . ';' . $css['css_added_to'] . ';' . $css['css_app'] . ';' . $css['css_location'] . ';' . $css['css_path'] . ';' . $css['css_name'] );
	...
	public function saveTemplate( $data )
	{
		...
		/* Unique key */
		$template['template_unique_key'] = md5( $data['set_id'] . ';' . $template['template_added_to'] . ';' . $template['template_app'] . ';' . $template['template_location'] . ';' . $template['template_group'] . ';' . mb_strtolower( $template['template_name'] ) );
	...
	<a rel='nofollow' title='Invision Community' href='#'>Powered by Invision Community</a><br><a href='#' style='display:none'>Powered by Invision Community</a>
</p>";
		}
			...
			if ( isset( $data['_default_template'] ) and $check['template_added_to'] > 0 )
			{
				...
				try
				{
					\IPS\Db::i()->update( 'core_theme_templates', array(
						...
						'template_unique_key' => md5( $check['template_added_to'] . ';' . mb_strtolower( $data['app'] ) . ';' . mb_strtolower( $data['location'] ) . ';' . mb_strtolower( $data['group'] ) . ';' . mb_strtolower( $data['name'] ) )
